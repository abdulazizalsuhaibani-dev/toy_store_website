...
- **`apps/toymodule/`** — the single Django app holding all business logic:
  ...
  - `views.py` — function-based views only (no CBVs, no DRF/API layer). `_catalogue()` is the shared filter/sort helper behind the listing and search screens. `dashboard`, `accountInfo` and `orders` are gated with `@login_required(login_url='login')`; `addProduct` needs the `toymodule.add_product` permission (`permission_required` in `views.py`: anonymous → login, customer → 403). Roles are the `Customer` and `Admin` auth groups from migration 0019 (`roles.py`); signup assigns Customer, and Admin is granted only from the Django admin.
  ...
  - `tests.py` — 63 tests covering currency conversion, the filters, the cart, checkout totals, and who may read an order confirmation.
