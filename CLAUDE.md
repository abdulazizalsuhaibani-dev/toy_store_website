...
- **`apps/toymodule/`** — the single Django app holding all business logic:
  ...
  - `strings.py` — every UI string as an `(English, Arabic)` pair, flattened per request into `{{ t.someKey }}`. A dict rather than gettext because the site is two languages chosen by a header toggle, not by `Accept-Language`. A number before a noun goes through `plural()` / `{% count n "toy" %}` instead (Arabic has five forms: لعبة واحدة, لعبتان, 3 ألعاب, 11 لعبة, 100 لعبة) — add the noun to `PLURALS`, never build `{{ n }} {{ t.toys }}`. `err_<code>` keys are form errors and `status_<value>` keys order statuses, both looked up by prefix; a test fails if any other key goes unused.
  ...
  - `templatetags/happybox.py` — `{% money %}`, `{% money_in %}` (an order's recorded currency), `{% label %}`/`{% blurb %}`/`{% payment_label %}` (language-aware model copy), `{% count %}` (a counted noun), `{% copy "key" n=... %}` (a string with placeholders), `{% when %}` (a date with the month name in the page's language — never the `date` filter, which prints English months), and `{% querystring %}` (rebuild the query string with one filter replaced).
  - `views.py` — function-based views only (no CBVs, no DRF/API layer). `_catalogue()` is the shared filter/sort helper behind the listing and search screens. `dashboard`, `accountInfo` and `orders` are gated with `@login_required(login_url='login')`; `addProduct`, the category screens (`dashboard/categories`) and the order-management screens (`dashboard/manage/orders`, status changes stamp `Order.status_changed_at`) each need their own `toymodule.*` permission (`permission_required` in `views.py`: anonymous → login, customer → 403). Roles are the `Customer` and `Admin` auth groups from migration 0019 (`roles.py`); signup assigns Customer, and Admin is granted only from the Django admin.
  - `forms.py` — `AddProductForm` (explicit field list, not `__all__`), `AddUserForm`, `LoginForm`, `CheckoutForm`, `CategoryForm`, `OrderStatusForm`. All mix in `StorefrontForm` and take `lang=`: labels and help come from `strings.py` (`copy_labels`/`copy_help`), and after validation every error is rewritten by its code to `err_<field>_<code>` or `err_<code>`, so Django's English ("This field is required.", password validators) never reaches the page. A new error should raise `ValidationError(..., code=...)` and get an `err_` key. Django's own i18n is deliberately not activated: it would also switch Arabic decimal formatting to `4,9`. Auth is Django's built-in `django.contrib.auth` — no custom User model.
  ...
  - `tests.py` — 152 tests covering currency conversion, the filters, the cart, checkout totals, who may read an order confirmation, and that no Django English leaks onto an Arabic page.
- **`apps/templates/`** — the single template root (`TEMPLATE_DIR` in `settings.py`, `APP_DIRS` also on). `layouts/base.html` is the shared layout; `includes/` holds `header.html`, `footer.html`, `logo.html`, `mascot.html`, `product_card.html`, `product_image.html` and `empty_shelf.html`; `toymodule/` holds the page templates; `404.html` (full storefront context) and `500.html` (standalone, bilingual, no context or database) sit at the root and only show with `DEBUG` off. When adding a page, extend `layouts/base.html` and add a template under `apps/templates/toymodule/`.
...
- **Listing and search are paginated** (`PAGE_SIZE` = 12, `_paginate()` in `views.py`, pager in `includes/pagination.html`). The views hand templates a `Page` as `products`, so use `page_obj.paginator.count` for "how many matched", not `products|length`. `{% querystring %}` drops `page` whenever a filter changes and keeps it only when `page=` is passed explicitly.
- **Stock** is `Product.quantity`; `in_stock` is a property over it, so there is one source of truth. The cart clamps to it, and checkout takes it with a conditional `UPDATE ... WHERE quantity >= n` inside the same transaction as the `Order`, so two checkouts of the last toy cannot both win and a short line rolls the whole order back. `/addProduct` takes the price in a chosen currency and converts with `Currency.to_base()`; what is stored is still base-currency only. `rating`/`review_count` are not form fields.
